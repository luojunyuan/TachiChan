...
3. 去掉 Gesture 对 winforms 的引用，移动到 AssistiveTouch.Core 命名空间下

使用多个进程的结构是有理由的
1. 仅在AssistiveTouch启动时读入用户配置，Preference对配置进行编辑。（AssistiveTouch结束时存储按钮位置是个例外）
2. Child window的结构会让touch随着游戏窗口的销毁而销毁，部分游戏存在启动器销毁，全屏切换销毁window handle的情景。所以需要常驻一个main进程（TouchChan）来重启AssistiveTouch。
3. Gesture 引用了winforms，为保持纯净必须与wpf项目分离开。
